...
export default function CameraScreen() {
  ...
  const pickImage = async (useCamera: boolean) => {
    ...
    } catch (e) { Alert.alert('Hata', 'Fotoğraf alınamadı'); }
  };

  const handleProceed = () => {
    if (!image) return;
    router.push({ pathname: '/analysis/loading', params: { category: category || 'medikal', photoBase64: image } });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{category === 'cerrahi' ? t('surgical') : t('medicalAesthetic')}</Text>
        <View style={{ width: 40 }} />
      </View>
          ...
          {!image ? (
            <>
              <TouchableOpacity testID="take-photo-btn" onPress={() => pickImage(true)} activeOpacity={0.8}>
                <LinearGradient colors={['#F3D088', '#D1A354']} style={styles.actionBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  scanFrame: { width: 260, height: 340, justifyContent: 'center', alignItems: 'center', gap: 16 },
  ...
  secondaryBtn: { borderRadius: RADIUS.md, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, borderWidth: 1, borderColor: COLORS.surface.glassBorder },
