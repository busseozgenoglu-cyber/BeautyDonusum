      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileCard}>
          ...
          <View style={[styles.subBadge, user?.subscription === 'premium' && styles.subBadgePremium]}>
            <Ionicons name={user?.subscription === 'premium' ? 'diamond' : 'person'} size={14} color={user?.subscription === 'premium' ? COLORS.brand.primary : COLORS.text.secondary} />
            <Text style={[styles.subText, user?.subscription === 'premium' && styles.subTextPremium]}>{user?.subscription === 'premium' ? t('premium') : t('free')}</Text>
          </View>
        </View>
        ...
        {user?.subscription !== 'premium' && (
          <TouchableOpacity testID="upgrade-btn" onPress={handleUpgrade} activeOpacity={0.8}>
            <LinearGradient colors={['#F3D088', '#D1A354']} style={styles.upgradeBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  profileCard: { alignItems: 'center', backgroundColor: COLORS.surface.card, borderRadius: RADIUS.lg, padding: 28, borderWidth: 1, borderColor: COLORS.surface.glassBorder, marginBottom: 20 },
  avatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: COLORS.bg.tertiary, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  ...
  subBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 12, paddingHorizontal: 14, paddingVertical: 6, borderRadius: RADIUS.full, backgroundColor: COLORS.surface.glass, borderWidth: 1, borderColor: COLORS.surface.glassBorder },
  ...
  statCard: { flex: 1, backgroundColor: COLORS.surface.card, borderRadius: RADIUS.md, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: COLORS.surface.glassBorder },
  statValue: { ...FONT.h2, color: COLORS.brand.primary },
  statLabel: { ...FONT.xs, color: COLORS.text.secondary, marginTop: 4 },
  upgradeBtn: { borderRadius: RADIUS.md, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 24 },
  ...
  settingRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: COLORS.surface.glassBorder },
